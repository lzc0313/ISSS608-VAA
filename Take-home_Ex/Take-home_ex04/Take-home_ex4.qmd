...
```{r}
pacman::p_load(sf,terra,gstat,tmap,viridis,tidyverse,dplyr,naniar,imputeTS, DT, knitr, lubridate,
               ggplot2, patchwork, ggthemes,
               tseries, ggHoriPlot,
               TSclust, fable, dtwclust, dendextend,
               ggraph, plotly, factoextra, ggdendrosf,)
```
...
colnames(weather_data_imputed)[8] <- "Min_Temperature"
colnames(weather_data_imputed)[9] <- "Max_Temperature"
```
...
#weather_data_2023 <- weather_data_imputed %>% 
#  filter(Year == 2023) 
...
ori <- sum(range(cutpoints$`Mean_Temperature`))/2
sca <- seq(range(cutpoints$`Mean_Temperature`)[1], 
           range(cutpoints$`Mean_Temperature`)[2], 
           length.out = 7)[-4]
...
for the control pannel there should be check box that affecting the model, not just displaying the changes
